...
def _scan_makefile(lines: List[str], target_name: Optional[str] = None) -> Union[Dict[str, int], int]:
...
class MakeRecipe:
    ...
    def describe(self, color: bool = False) -> List[str]:
        ...
        if self.dependencies:
            deps_str = " ".join(f"{c.MAGENTA}{dep}{c.RESET}" for dep in self.dependencies)
    ...
    if args.all:
        ...
        for tgt in args.targets:
            ...
            if any(char in tgt for char in ['*', '?', '[']):
                ...
                matched_targets: List[str] = [t for t in all_targets if fnmatch.fnmatch(t, tgt)]
                if matched_targets:
                    for matched in matched_targets:
                        recipes.append(MakeRecipe.from_makefile(lines, matched))
                else:
                    print(f"Error: no targets match pattern '{c.RED}{tgt}{c.RESET}'", file=sys.stderr)
                    sys.exit(1)
            else:
                # Exact target lookup
                try:
                    ...
                    fuzzy_matches: List[str] = difflib.get_close_matches(tgt, all_targets, n=5, cutoff=0.6)
                    # Also find targets that contain the attempted target
                    substring_matches: List[str] = [t for t in all_targets if tgt in t and t not in fuzzy_matches]
                    # Combine and deduplicate while preserving order
                    matches: List[str] = fuzzy_matches + substring_matches
                    matches = matches[:5]  # Limit to 5 suggestions

                    print(f"Error: target '{c.RED}{tgt}{c.RESET}' not found in makefile", file=sys.stderr)
                    if matches:
                        suggestions: str = ", ".join(f"{c.BLUE}{m}{c.RESET}" for m in matches)
